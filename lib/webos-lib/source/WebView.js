...
enyo.kind({
    ...
    touchHandler: function(inSender, inEvent) {
        this.log("Touch event: ", inEvent.type);
        // nop
    },
    ...
    resize: function() {
        ...
        this.log("Resizte to: ", JSON.stringify(s));
    ...
    getBounds: function() {
        ...
        var bounds = this.owner.owner.getBounds(),
            ...
            ownerBorderWidth = 0;
        this.log("Getting bounds from ", this.owner.owner.name, " bounds: ", JSON.stringify(bounds));
    ...
    cacheBoxSize: function() {
        ...
        this.log("Setting chacheBoxSize to ", JSON.stringify(this._boxSize));
    ...
    updateViewportSize: function() {
        ...
        if (b.width && b.height) {
            this.log("Setting view port size to ", JSON.stringify(b));
    ...
    dispatchDomEvent: function(inEvent) {
        this.log("inEvent.type: ", inEvent.type);
        var r = true;
    ...
    flickHandler: function(inSender, inEvent) {
        ...
        this.log("FlickHandler");
    },
    scrollHandler: function(inSender, inEvent) {
        //this.callBrowserAdapter("handleScroll", [inEvent.xVel, inEvent.yVel]);
        if (!webos.isPhone()) {
            // prevent flick event from bubbling when flicking in webview
            return true;
        }
        this.log("ScrollHandler: ", inEvent);
    },
    clickHandler: function(inSender, inEvent) {
        this.log("click handler");
        for (var x in inEvent) {
            this.log("inEvent[" + x + "] = " + inEvent[x]);
        }
        if(webos.isPhone()) {
            this.callBrowserAdapter("clickAt", [inEvent.offsetX * window.zoomFactor, event.offsetY * window.zoomfactor, 0]);
        }
    },
    ...
    **/
    eventFired: function(inEvent, inInfo) {
        this.log("Event fired: ", inEvent, " with ", inInfo);
    ...
    titleURLChange: function(inTitle, inUrl, inCanGoBack, inCanGoForward) {
        this.log("Title / URL change: ", inTitle, ", ", inUrl, ", ", inCanGoBack, ", ", inCanGoForward);
...
enyo.kind({
    ...
    pageTitleChanged: function(inSender, inEvent) {
        ...
        this.log("Sending pageTitleChanged ", inEvent);
